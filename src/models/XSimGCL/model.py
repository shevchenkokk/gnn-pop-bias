...
class XSimGCL(BasicModel):
    """
    XSimGCL: Towards Extremely Simple Graph Contrastive Learning for Recommendation
    
    Reference: 
    Yu, J., Yin, H., Gao, M., Xia, X., Zhang, X., & Vucetic, S. (2022). 
    Towards Extremely Simple Graph Contrastive Learning for Recommendation.
    """
    def __init__(
        self,
        ...
        self.n_layers = self.config["XSimGCL_n_layers"]
        self.decay = self.config.get("decay", 1e-4)
        self.device = self.config.get("device", "cuda" if torch.cuda.is_available() else "cpu")
        
        # XSimGCL specific parameters
        ...
        self.noise_scale = self.config.get("noise_scale", 0.1)  # Noise magnitude for augmentation
        self.proto_reg = self.config.get("proto_reg", 0.1)  # Prototype regularization weight
        self.n_clusters = self.config.get("n_clusters", 10)  # Number of clusters for prototypes
    ...
    def __init_weight(self):
        ...
        print("--- XSimGCL is ready to go ---")
        
    def __init_prototypes(self):
        """
        Initialize prototypes for user and item clusters
        """
        # Initialize prototypes for users and items
        self.user_protos = nn.Parameter(
            torch.randn(self.n_clusters, self.embedding_dim).to(self.device)
        )
        self.item_protos = nn.Parameter(
            torch.randn(self.n_clusters, self.embedding_dim).to(self.device)
        )
        
        # Initialize with normal distribution
        nn.init.normal_(self.user_protos, std=0.1)
        nn.init.normal_(self.item_protos, std=0.1)
        
    def _add_noise(self, embeddings):
        """
        Add noise to embeddings following the XSimGCL paper
    ...
    def _contrastive_loss(self, view1, view2):
        """
        Compute contrastive loss between two views following the XSimGCL paper
        ...
        # Compute positive scores - dot product of corresponding embeddings
    ...
    def _prototype_loss(self, user_embs, item_embs):
        """
        ...
        user_protos = F.normalize(self.user_protos, p=2, dim=1)
        item_protos = F.normalize(self.item_protos, p=2, dim=1)
        
        # Compute similarities between embeddings and prototypes
        user_proto_sim = torch.matmul(user_embs, user_protos.t())  # [n_users, n_clusters]
        item_proto_sim = torch.matmul(item_embs, item_protos.t())  # [n_items, n_clusters]
        
        # Compute soft cluster assignments (softmax)
        user_cluster_dist = F.softmax(user_proto_sim, dim=1)  # [n_users, n_clusters]
        item_cluster_dist = F.softmax(item_proto_sim, dim=1)  # [n_items, n_clusters]
        
        # Compute prototype loss - encourage diversity in cluster assignments
        user_proto_loss = -torch.mean(torch.sum(user_cluster_dist * torch.log(user_cluster_dist + 1e-10), dim=1))
        item_proto_loss = -torch.mean(torch.sum(item_cluster_dist * torch.log(item_cluster_dist + 1e-10), dim=1))
        
        return user_proto_loss + item_proto_loss
    
    def compute_loss(self, users: torch.Tensor, pos: torch.Tensor, neg: torch.Tensor) -> dict:
        """
        Compute BPR loss with contrastive learning and prototype regularization
        ...
        # Prototype regularization loss
        all_users, all_items = self.propagate()
        proto_loss = self._prototype_loss(all_users, all_items)
        
        # Total Loss
        total_loss = bpr_loss + self.decay * reg_loss + self.cl_rate * cl_loss + self.proto_reg * proto_loss
